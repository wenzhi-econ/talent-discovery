...
Inputs:
(a) Compact Parquet tables created by ``C02_DataPrep_MatchRates_AggTables.py`` in
    data/b_temp_data/B01_ConstructAnalysisSample/C02_MatchRatesToInventorData.
...
(3) Every figure has its own local controls; country comparisons are fixed to all countries,
...
def imports():
    ...
    return alt, mo, pd, pl, px, pycountry, re
...
def helpers(alt, pd, pl, pycountry, re):
    ...
    def aggregate_classification_rates(data, value_column, title_column=None):
        _summary = add_rate_statistics(data.copy())
        _summary["group_value"] = _summary["value"].map(_display_value).astype("string")
        _summary[value_column] = _summary["group_value"]
        if title_column is None:
            _summary["display_label"] = _summary["group_value"]
        else:
        ...
        if "scope_key" in _summary:
            _summary["country_scope"] = _summary["scope_key"].map(_scope_labels)
        return _summary

    def aggregate_reference_rates(scope_totals, metric):
        _summary = add_rate_statistics(scope_totals.copy())
        _rate_column = "spell_match_rate" if metric == "spell" else "user_match_rate"
        _labels = {
        ...
        return pd.DataFrame(
            {
                "reference_scope": _summary["scope_key"].map(_labels),
                "scope_detail": _summary["scope_key"].map(_details),
                "scope_key": _summary["scope_key"],
                "reference_rate": _summary[_rate_column],
            }
        )
...
def paths_and_schema(hierarchy_number, mo, pd):
    ...
    REQUIRED_AGGREGATES = (
        ...
        "scope_totals.parquet",
        "country_rates.parquet",
        "us_state_rates.parquet",
    )
    missing_aggregates = [
    ...
    if missing_aggregates:
        raise FileNotFoundError(
            "Run C02_DataPrep_MatchRates_AggTables.py first. "
...
def basic_numbers(
    ...
    _totals = add_rate_statistics(scope_totals.copy())
    _rows = []
    for _row in _totals.itertuples(index=False):
        _scope_label = _scope_labels[_row.scope_key]
        _rows.extend(
            [
                {
                    "Country scope": _scope_label,
                    "Observation level": "User-company",
                    "Candidate observations": _row.candidate_spells,
                    "Matched observations": _row.matched_spells,
                    "Match rate": _row.spell_match_rate,
                },
                {
                    "Country scope": _scope_label,
                    "Observation level": "User",
                    "Candidate observations": _row.unique_users,
                    "Matched observations": _row.matched_users,
                    "Match rate": _row.user_match_rate,
                },
            ]
        )
            ...
            mo.accordion(
                {
                    ...
                    "Country coverage": mo.ui.table(
                        pd.DataFrame(
                            {"Nonmissing countries": [int(metadata["nonmissing_country_count"])]}
                        ),
                        pagination=False,
                        show_column_summaries=False,
                    ),
...
def occupation_top_n_control(
    ...
    _table = load_rate_counts(_occupation_column)
    _category_count = int(
        _table.loc[
            (_table["scope_key"] == "all") & (_table["value"].astype(str) != MISSING_LABEL),
            "value",
        ].nunique()
    )
...
def occupation_rates(
    ...
    occupation_summary = aggregate_classification_rates(
        load_rate_counts(occupation_column),
        occupation_column,
        OCCUPATION_TITLES.get(occupation_column),
    )
    occupation_reference_table = aggregate_reference_rates(scope_totals, occupation_metric)
...
def industry_top_n_control(
    ...
    _table = load_rate_counts(_industry_column)
    _category_count = int(
        _table.loc[
            (_table["scope_key"] == "all")
            & (_table["value"].astype(str) != MISSING_LABEL)
            & (_table["candidate_spells"] >= MIN_ALL_COUNTRY_INDUSTRY_HIRES),
            "value",
        ].nunique()
    )
    _max_categories = max(1, _category_count)
...
def industry_rates(
    ...
    industry_summary = aggregate_classification_rates(
        load_rate_counts(industry_column),
        industry_column,
        INDUSTRY_TITLES.get(industry_column),
    )
    industry_reference_table = aggregate_reference_rates(scope_totals, industry_metric)
    ...
    industry_plot_table = industry_plot_table.drop(columns=["scope_key"], errors="ignore")
...
def industry_occupation_category_controls(
    ...
    io_industry_category_table = aggregate_classification_rates(
        load_rate_counts(io_industry_column).loc[lambda data: data["scope_key"] == "all"],
        io_industry_column,
    ...
    io_occupation_category_table = aggregate_classification_rates(
        load_rate_counts(io_occupation_column).loc[lambda data: data["scope_key"] == "all"],
    ...
    _default_industry_labels = [
        _industry_label_by_value[value]
        for value in _default_industry_values
        if value in _industry_label_by_value
    ]
    ...
    if io_occupation_column == "onet_code":
        ...
        for title in _default_occupation_titles:
            matches = io_occupation_category_table.loc[
                io_occupation_category_table[_occupation_title_column] == title,
                "display_label",
            ]
            if not matches.empty:
                _default_occupation_labels.append(matches.iloc[0])
    else:
...
def industry_occupation_rates(
    ...
    _industry_values = [str(value) for value in io_industry_values_selector.value]
    _occupation_values = [str(value) for value in io_occupation_values_selector.value]
    _scope_key = io_country_scope_selector.value
    _scope_labels = {value: label for label, value in SCOPE_OPTIONS.items()}
    ...
    if _industry_values and _occupation_values:
        _summary = load_joint_counts(io_industry_column, io_occupation_column)
        _summary = _summary.loc[
            (_summary["scope_key"] == _scope_key)
            & _summary["industry_value"].astype(str).isin(_industry_values)
            & _summary["occupation_value"].astype(str).isin(_occupation_values)
        ].copy()
        _summary = add_rate_statistics(_summary)
        ...
        for column in (
            ...
            _grid[column] = _grid[column].fillna(0).astype(int)
        ...
        _candidate_total = int(_grid["candidate_count"].sum())
        _matched_total = int(_grid["matched_count"].sum())
        industry_occupation_totals = pd.DataFrame(
            [
                {
                    "Country scope": _scope_label,
                    ...
                    "Candidate observations": _candidate_total,
                    "Matched observations": _matched_total,
                    "Match rate": (
                        _matched_total / _candidate_total if _candidate_total else float("nan")
                    ),
                }
            ]
        )
        ...
        if _metric == "user":
            industry_occupation_note = (
                " Pooled user totals sum cell-specific distinct users; a user in "
                "multiple selected cells is counted once per cell because the "
                "privacy-safe aggregates contain no user identifiers."
            )
    return (
...
def country_rates(
    ...
    country_chart, country_plot_table = make_single_rate_chart(
        country_summary,
        (
            f"Inventor {metric_label(country_metric)}-level match rates across "
            f"economies with at least {MIN_COUNTRY_CANDIDATE_SPELLS:,} "
            "candidate observations"
        ),
        country_metric,
    ...
    if mapped_country_summary.empty:
        ...
        country_map = px.choropleth(
            ...
            title=(
                f"Inventor {metric_label(country_metric)}-level match rates "
                f"across economies with at least "
                f"{MIN_COUNTRY_CANDIDATE_SPELLS:,} candidate observations"
            ),
        )
...
def seniority_rates(
    ...
    seniority_summary = aggregate_classification_rates(load_rate_counts("seniority"), "seniority")
    seniority_reference_table = aggregate_reference_rates(scope_totals, seniority_metric)
    ...
    seniority_plot_table = seniority_plot_table.sort_values(
        ["seniority_order", "country_scope"], na_position="last"
...
def start_month_rates(
    ...
    start_month_summary = aggregate_classification_rates(
        load_rate_counts("start_month"), "start_month"
    )
    start_month_summary["month"] = pd.to_datetime(start_month_summary["value"], errors="coerce")
    start_month_summary["start_month"] = start_month_summary["month"]
    start_month_chart, start_month_plot_table = make_time_chart(
        start_month_summary,
        (
            f"Monthly inventor {metric_label(start_month_metric)}-level "
