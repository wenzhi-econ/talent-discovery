...
"""
Task:
    Describe the inventor-matched subsample of candidate focal new hires.
...
(a) Interactive marimo views of occupation, industry, and geographic distributions
    among inventor-matched focal-hire spells.

Notes:
(1) The unit is a focal-hire spell: one retained spell per user-company pair.
(2) A spell is retained when its user_id appears at least once in the inventor crosswalk.
(3) The crosswalk is collapsed to unique, nonmissing user IDs before the semi-join, so
    users linked to multiple inventor IDs do not duplicate focal-hire spells.
(4) Only requested analysis columns are loaded; the lazy semi-join filters rows before
    the matched subsample is collected into pandas memory.
(5) The notebook detects absent role_k* and rics_k* fields and reports them explicitly.
(6) The U.S. state map uses Plotly's built-in USA-states geometry.

Run:
    $fnh_notebook = "codes/B01_ConstructAnalysisSample/B04_FNH_MatchedSummaryStats.py"
...
def _(mo):
    mo.md(r"""
    # Inventor-matched candidate focal new hires

    This notebook redoes the occupation, industry, and geographic descriptions for the
    subsample whose users can be linked to a USPTO inventor identifier. The unit remains
    a **focal-hire spell**: one retained spell per user-company pair. Counts and shares
    are calculated within the inventor-matched subsample.

    Missing classifications are retained as explicit categories. A crosswalk match is a
    linkage/coverage indicator; it is not itself a measure of inventive productivity.
    """)
    return
    ...
    def make_bar_chart(summary, title, top_n):
        ...
        _base = alt.Chart(_top).encode(
            ...
            tooltip=[
                ...
                alt.Tooltip(
                    "count:Q",
                    title="Inventor-matched focal-hire spells",
                    format=",",
                ),
        ...
        _bars = _base.mark_bar(color="#2563EB", opacity=0.85).encode(
            x=alt.X(
                "share:Q",
                title="Share of inventor-matched focal-hire spells",
...
@app.cell(hide_code=True)
def _(ds, hierarchy_number, mo, pd, pl):
    INPUT_DIR = (
    ...
    _link_user_counts = _patent_links.drop_nulls("user_id").group_by("user_id").len()
    _patent_users = _patent_links.select("user_id").drop_nulls().unique()
    if _patent_users.is_empty():
        raise ValueError("Inventor crosswalk contains no nonmissing user IDs.")

    _matched_fnh = (
        pl.scan_parquet(str(INPUT_DIR / "*.parquet"))
        .select(list(ANALYSIS_COLUMNS))
        .join(_patent_users.lazy(), on="user_id", how="semi")
        .collect(engine="streaming")
    )
    if _matched_fnh.is_empty():
        raise ValueError("No focal-hire spells match a user in the inventor crosswalk.")

    fnh = _matched_fnh.to_pandas(use_pyarrow_extension_array=True)
        ...
        [
            {
                ...
                "Unique inventor IDs": (
                    _patent_links["pv_inventor_id"].drop_nulls().n_unique()
                ),
                ...
                "Matched focal-hire spells": len(fnh),
    ...
    mo.vstack(
        [
            mo.md(
                ...
                inventor-matched focal-hire spells as the denominator, including
...
def _(onet_role_pairs):
    ...
    if not role_onet_pairs.empty:
        role_onet_pairs["share_within_role"] = role_onet_pairs[
            "count"
...
def _(
    ...
    _industry_coverage = selected_industry_table.head(industry_top_n.value)[
        "share"
    ].sum()
...
def _(AVAILABLE_RICS_COLUMNS, MISSING_LABEL, fnh, pd):
    ...
    if finest_rics_column is None:
        ...
        _crosswalk = fnh[[finest_rics_column, "naics_code", "naics_description"]].copy()
...
def _(rics_naics_pairs):
    ...
    if not naics_rics_pairs.empty:
        ...
        naics_rics_pairs["share_within_naics"] = naics_rics_pairs[
            "count"
...
def _(mapped_country_summary, mo, px, unmapped_country_summary):
    ...
    world_country_map = px.choropleth(
        ...
        labels={
            "count": "Inventor-matched focal-hire spells",
            "share": "Global share",
            "log10_count": "log10 spells",
        },
        color_continuous_scale="Blues",
        projection="natural earth",
        title="Inventor-matched focal-hire spells by country",
    )
    ...
    mo.vstack(
        [
            world_country_map,
            mo.md(
                f"ISO mapping covers **{_iso_coverage:.2%}** of "
                "inventor-matched focal-hire spells."
            ),
            mo.accordion(
                {
...
def _(mo):
    ...
    inventor-matched focal-hire spells as the denominator, including missing or
    unmapped state labels.
    """)
    return
...
def _(px, us_state_code, us_state_summary):
    ...
    state_map_coverage = state_map_data["count"].sum() / _us_total if _us_total else 0.0
    ...
    if state_map_data.empty:
        ...
        state_map_figure = px.choropleth(
            ...
            color_continuous_scale="Blues",
            title="Inventor-matched focal-hire spells by U.S. state",
        )
    ...
    mo.vstack(
        [
            ...
            mo.md(
                ...
                "U.S. inventor-matched focal-hire spells."
            ),
            mo.accordion(
                {
...
def _(
    ...
    _naics_conflicts = int((naics_title_diagnostic["distinct_descriptions"] > 1).sum())
    mo.vstack(
        [
            mo.md(
                f"""
    ...
    mo.vstack(
        [
            mo.md(
                ...
                - **Inventor-matched focal-hire spells:** {len(fnh):,}
                - **Matched users represented:** {fnh["user_id"].nunique(dropna=True):,}
                - **Source Parquet parts:** {len(PARQUET_FILES):,}
                - **Countries:** {fnh["country"].nunique(dropna=True):,}
                - **Pandas memory for matched selected columns:** {_memory_mib:,.1f} MiB
                - **Missing marker in charts:** `{MISSING_LABEL}`

                The semi-join uses unique nonmissing crosswalk user IDs. Multiple
                PatentView inventor IDs for the same Revelio user therefore cannot
                duplicate a user-company focal-hire spell.
                """
            ),
            _schema_callout,
            mo.accordion(
                {
                    "View inventor-crosswalk diagnostics": mo.ui.table(
                        link_diagnostics,
                        pagination=False,
                        show_column_summaries=False,
                    ),
...
def _(mo):
    mo.callout(
        mo.md(r"""
        **Interpretation boundary.** Crosswalk inclusion combines inventive activity,
        PatentView's USPTO coverage, and user-to-inventor linkage quality. The
        distributions in this notebook therefore describe the observed matched
        subsample, not all inventors or the full candidate-new-hire universe. Differences
        across occupations, industries, and locations may partly reflect linkage
        selection and should guide—not mechanically determine—the formal sample choice.
        """),
        kind="warn",
    )
    return
